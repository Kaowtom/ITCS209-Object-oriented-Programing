...
public class Kalculator {
	...
	private double sum,mean,std,max,min; // for calculating
	private double[] maxK,minK; // for storing data as Array
	//**************************************
	/**
	...
	{
		//******INSERT YOUR CODE HERE***********
		sum=0;mean=0;std=0;max=0;min=0;
		//**************************************
	}
	...
	public void deleteLastNumber()
	{
		//******INSERT YOUR CODE HERE***********
		if(data.size()!=0) {
			data.remove(data.size()-1);} // Remove the last number 
		//**************************************
	}
	...
	public double getSum()
	{
		...
		for(int i=0;i<data.size();i++) {
			sum+=data.get(i);} // retrieve data for using 
			return sum;}
		return 0;
		//**************************************
	}
	...
	public double getStd()
	{
		...
		if(data.size()>2) {
			for(int i=0;i<data.size();i++) {
				std+=Math.pow(data.get(i)-mean, 2);
			} 
			std=std/(data.size()-1);
			std=Math.sqrt(std);
			return std;
	...
	public double[] getMaxK(int k)
	{
		...
		if(data.size()>k) {
			...
			for(int i=0;i<data.size();i++) {
				sorting.add(data.get(i));} // Create and retrieve new ArrayList from the data
			Collections.sort(sorting); // Sort number from ArrayList
			maxK=new double [k]; // Assign maxK as an array of K number 
			for(int j=0;j<k;j++) {
				maxK[j]=sorting.get(sorting.size()-(j+1));
			}	
			return maxK;
		}
		return null;
