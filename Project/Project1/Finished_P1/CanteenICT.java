...
//	private List<Customer> waitToEnterQueueNext = new ArrayList<Customer>();
//	private List<Customer> waitToSeatQueueNext = new ArrayList<Customer>();
	
	// list to getting waittoenterqueue
	public List<Customer> getWaitToEnterQueue() {
		return waitToEnterQueue;
	}
	
	// list to getting waittoseatqueue
	public List<Customer> getWaitToSeatQueue() {
		return waitToSeatQueue;
	}
	
	// list to getting foodstalls
	public List<FoodStall> getfoodStalls() {
		return foodStalls;
	}
	
	public static boolean AvaAdd = true;
	...
	{	
		...
		//************************************** YOUR CODE HERE *********************************//
		int total=0; //set new variable to collect sum of price
		for(FoodStall fs : this.foodStalls) {
			List<FoodStall.Menu> menu = fs.getMenu(); // set menu to define the location of Menu that we want to use
			for(FoodStall.Menu dish: menu) {
				total += FoodStall.INSTALLATION_COST[dish.ordinal()]; // collect that menu price in total variable
			}
		}
	...
	public boolean validateCanteen()
	{
		...
		//************************************** YOUR CODE HERE *********************************//
		//Using boolean method to make the condition easier  \( O w O )/
		boolean t1 = true, t2 = false, t3 = true;
		
		//check the method does all cost > max budget?
		if(getInstallCost() > MAX_BUDGET) {
			t1 = false;
		}

		// check that all food store sell all menu
		for(FoodStall fs : this.foodStalls) {
			
			//by using size of menu of this food store are the same of base food menu
			if(fs.getMenu().size() == FoodStall.Menu.values().length) {
				t2=true;
			}
		}
		
		// check that do we have some table 
		if(tables.size() == 0) {
			t3 = false;
		}
		
		// if everything are true it's mean we are ready to open the canteen
		if(t1 && t2 && t3) {
			return true;
		}else {
			return false;
	...
	private void preprocess()
	{
		//******************************************** YOUR CODE HERE (IF ANY) *******************************//
			// I can sware to god I already try it ._.
		//****************************************************************************************************//
	}
	...
	{
		//******************************************** YOUR CODE HERE (IF ANY) *******************************//
		// This thing is too hard for me ._."
		//****************************************************************************************************//
